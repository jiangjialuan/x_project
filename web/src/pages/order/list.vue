      ...
      <div>
        <el-form :inline="true" :model="form" class="demo-form-inline">
          <el-form-item label="用户帐号">
            <el-input v-model="form.mobile_phone" placeholder="用户帐号"></el-input>
          </el-form-item>
        ...
        <div>
          <table style="width:100%;" cellspacing="0" cellpadding="0" border="0">
            <tr>
              <th class="th" width="27%" style="min-width:266px;">商品</th>
              <th class="th" width="13%">兑换时间</th>
              <th class="th" width="20%" >兑换人信息</th>
              <th class="th" width="20%">发货状态</th>
              <th class="th" width="20%">兑换积分</th>
            </tr>
          </table>
          <el-card v-if="tableData.length>0" class="box-card" shadow="never" v-for="(item,index) in tableData" :key="'order'+index" >
            ...
            <table style="width:100%;" cellspacing="0" cellpadding="0" border="0" class="el-table--border">
              <tr v-for="(gooditem,idx) in item.goods" :key="'good'+idx" v-if="idx<5">
                  ...
                  <div class="cell">
                    <div class="goods-header">
                      ...
                      <div class="goods-info">
                        <p class="goods-info-title">{{ gooditem.goods_name}}</p>
                      </div>
                    </div>
                  </div>
                </td>
                <td class="td" width="13%"  v-if="idx==0" :rowspan="item.goods.length">
                  <div class="cell">
                    <p v-if="item.exchange_time">{{item.exchange_time}}</p>
                    <p v-else>无</p>
                  </div>
                </td>
                <td class="td" width="20%" v-if="idx==0" :rowspan="item.goods.length">
                  <div class="cell">
                ...
                <td class="td" width="20%" v-if="idx==0" :rowspan="item.goods.length">
                  <div class="cell">
                    ...
                    <p class="state" v-if="item.shipping_status=='no'"><el-button size="mini" @click="finish_order(item.id,item.use_points)">标记为已发货</el-button></p>
                  </div>
                </td>
                <td class="td" width="20%" v-if="idx==0" :rowspan="item.goods.length">
                  <div class="cell">
              ...
              <tr v-if="item.goods.length>5">
                <td class="td" colspan="5">剩余{{item.goods.length-5}}项未显示，<el-button type="text" @click="goDetail(item.id)">查看详情</el-button></td>
              </tr>
            </table>
          </el-card>
